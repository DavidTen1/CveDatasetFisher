...
#Source for ast:https://www.geeksforgeeks.org/python-convert-a-string-representation-of-list-into-list/
import ast
...
#checks if text files exist, one for listing repos, the 2nd one listing key-value-pairs(repos as keys and their commit histories as values)
reposFileExists = os.path.exists('repos.txt')
reposCommitsFileExists = os.path.exists('reposCommits.txt')

reposInfo = None
reposCommitsInfo = None
reposList = reposInfo or []
reposCommitsList = reposCommitsInfo or []
lAlt = []
cvesList = []


#read out the repos' info and each their commit histories if possible, else create the 2 files
if reposFileExists:
 reposRead = open('repos.txt', 'r')
 reposInfo = reposRead.read()
 reposInfoArr = ast.literal_eval(reposInfo)
 reposList = reposInfoArr
else:
    reposRead = open('repos.txt', 'x')
    reposList = []
    reposRead.write(str(reposList))

if reposCommitsFileExists:
 reposCommitsRead = open('reposCommits.txt', 'r')
 reposCommitsInfo = reposCommitsRead.read()
 reposCommitsInfoArr = ast.literal_eval(reposCommitsInfo)
 reposCommitsList = reposCommitsInfoArr
else:
    reposCommitsRead = open('reposCommits.txt', 'x')
    reposCommitsList = []
    reposCommitsRead.write(str(reposCommitsList))





def updateReposCommitsInfo(repoName, cloneRequired = False, newInput = None):
    newRepoList = ast.literal_eval(reposInfo) if reposInfo and isinstance(reposInfo, str) else []
    #newReposCommitsList = reposCommitsList
    newReposCommitsList = ast.literal_eval(reposCommitsInfo) if reposInfo and isinstance(reposCommitsInfo, str) else []
    dict = {}
    #save the new repo and its commit history
    if(not cloneRequired):
     newRepoList.append(repoName)
     dict[repoName] = listCurrentRepoHistory(repoName)
     newReposCommitsList.append(dict)
     reposList = newRepoList
     reposCommitsList = newReposCommitsList
     with open('repos.txt', "w") as reposFile:
      reposFile.write(str(reposList))
     with open('reposCommits.txt', "w") as reposCommitsFile:
      reposCommitsFile.write(str(reposCommitsList))
    else:
        #just update the chosen repo's commit history do nothing else
        updatedDict = list(filter(lambda targetValue: repoName in targetValue, newReposCommitsList))
        if updatedDict:
         updatedDict[0][repoName] = newInput
         reposCommitsList = newReposCommitsList
         with open('reposCommits.txt', "w") as reposCommitsFile:
          reposCommitsFile.write(str(reposCommitsList))
...
class RepoUpdater:
    ...
    None
       ...
       updateReposCommitsInfo(repoName)
       restart_click()
    ...
    None
      ...
      defaultBranch = 'main' if  'main' in str(self.repo.branches) else 'master'
      self.repo.git.checkout(defaultBranch)
      self.repo.git.pull()
      newRepo = self.repo
      self.repo = newRepo
      #updates a repo's commit history
      newRepoCommitHistory = listCurrentRepoHistory(repoName)
      updateReposCommitsInfo(repoName,True,newRepoCommitHistory)
      restart_click()
